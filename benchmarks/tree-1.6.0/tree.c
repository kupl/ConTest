...
/* Until I get rid of this hack, make it linux/cygwin/HP nonstop only: */
  ...
  for(n=i=1;i<argc;i=n) {
	...
	case 'P':
	  if (argv[n] == NULL) {
	    fprintf(fp,"tree: missing argument to -P option.\n");
	...
	case 'I':
	  if (argv[n] == NULL) {
	    fprintf(fp,"tree: missing argument to -I option.\n");
	...
	case 'H':
	  ...
	  Xflag = FALSE;
	  if (argv[n] == NULL) {
	    fprintf(fp,"tree: missing argument to -H option.\n");
	...
	case 'T':
	  if (argv[n] == NULL) {
	    fprintf(fp,"tree: missing argument to -T option.\n");
	...
	case 'L':
	  if ((sLevel = argv[n++]) == NULL) {
	    fprintf(fp,"tree: Missing argument to -L option.\n");
	    fprintf(stderr,"tree: Missing argument to -L option.\n");
	    exit(1);
	  }
	  Level = strtoul(sLevel,NULL,0)-1;
	  if (Level < 0) {
	    fprintf(fp,"tree: Invalid level, must be greater than 0.\n");
	...
	case 'o':
	  if (argv[n] == NULL) {
	    fprintf(fp,"tree: missing argument to -o option.\n");
	...
	case '-':
	  if (j == 1) {
		...
		fprintf(fp,"tree: missing argument to --filelimit\n");
		...
		fprintf(fp,"tree: missing argument to --timefmt\n");
	...
	default:
	  fprintf(fp,"tree: Invalid argument -`%c'.\n",argv[i][j]);
  ...
  if (outfilename == NULL) {
    ...
    outfile = stdout;
		...
		outfile = stdout;
    if (outfile == NULL) {
      fprintf(fp,"tree: invalid filename '%s'\n",outfilename);
...
int alnumsort(struct _info **a, struct _info **b) __attribute__((crest_skip));
int versort(struct _info **a, struct _info **b) __attribute__((crest_skip));
int reversealnumsort(struct _info **a, struct _info **b) __attribute__((crest_skip));
int ctimesort(struct _info **a, struct _info **b) __attribute__((crest_skip));
int mtimesort(struct _info **a, struct _info **b) __attribute__((crest_skip));
...
void *xmalloc (size_t size)
{
  ...
  if (value == 0) {
    fprintf(fp,"tree: virtual memory exhausted.\n");
