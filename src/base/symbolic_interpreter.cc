...
value_t SymbolicInterpreter::NewInput(type_t type, addr_t addr) {
	...
	// Tokenize the template file.
	ifstream t_file;
	char line[20];
	static int p=0;
	int pos[20], t_char[20], i;
	t_file.open("template.txt");
	i=0;
	if(t_file.is_open()){
		while(t_file.getline(line, sizeof(line))){
			vector<string> tokens;
			split(tokens, line, boost::is_any_of(":"));
		 	pos[i] = atoi(tokens[0].c_str());
		 	t_char[i] = atoi(tokens[1].c_str());
			i++;
		}
	}
	t_file.close();
